...
class ErrorTest extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  errorTests(
    "can't define LET with the same name as already defined in scope" -> "already defined in the scope" -> BLOCK(
      LET(PART.VALID("X"), CONST_LONG(1)),
      BLOCK(LET(PART.VALID("X"), CONST_LONG(2)), TRUE)),
    "can't define LET with the same name as predefined constant" -> "already defined in the scope" -> BLOCK(LET(PART.VALID("None"), CONST_LONG(2)),
                                                                                                            TRUE),
    "can't define LET with the same name as predefined function" -> "function with such name is predefined" -> BLOCK(LET(PART.VALID("Some"),
                                                                                                                         CONST_LONG(2)),
                                                                                                                     TRUE),
    ...
    "FUNCTION_CALL with wrong amount of arguments" -> "requires 2 arguments" -> FUNCTION_CALL(PART.VALID(multiplierFunction.name),
                                                                                              List(CONST_LONG(0))),
    "FUNCTION_CALL with upper type" -> "Non-matching types" -> FUNCTION_CALL(PART.VALID(unitOnNone.name),
                                                                             List(FUNCTION_CALL(PART.VALID("Some"), List(CONST_LONG(3))))),
    "FUNCTION_CALL with wrong type of argument" -> "Typecheck failed: Non-matching types: expected: LONG, actual: BOOLEAN" -> FUNCTION_CALL(
      PART.VALID(multiplierFunction.name),
      List(CONST_LONG(0), FALSE)),
    "FUNCTION_CALL with uncommon types for parameter T" -> "Can't match inferred types" -> FUNCTION_CALL(
      PART.VALID(functionWithTwoPrarmsOfTheSameType.name),
      List(CONST_LONG(1), CONST_BYTEVECTOR(ByteVector.empty)))
  )
